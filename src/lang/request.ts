...
const addRequestReader = (
	cl: CommerceLayerProvisioningClient,
	interrupt = true,
): RequestReader => {
...
const isRequestInterrupted = (error: unknown): boolean => {
	return (
		CommerceLayerProvisioningStatic.isSdkError(error) &&
...
export const getFullUrl = (request: RequestData): string => {
	...
	if (request.params && Object.keys(request.params as object).length > 0) {
		const qs = Object.entries(request.params as object)
			.map(([k, v]) => `${k}=${v}`)
			.join("&");
		fullUrl += `?${qs}`;
	}
...
export const getOperation = (request: RequestData): OperationData => {
	...
	if (op.relationship)
		op.oneToMany = clText.pluralize(op.relationship) === op.relationship;

	if (op.method === "get") {
		if (singleton || (op.id && !op.relationship)) op.name = "retrieve";
		else op.name = rel || "list";
	} else if (method === "patch") op.name = "update";
	else if (method === "post") op.name = "create";
	else if (method === "delete") op.name = "delete";
