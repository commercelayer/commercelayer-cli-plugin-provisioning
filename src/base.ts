...
export abstract class BaseCommand extends Command {
	...
	protected checkApplication(accessToken: string, kinds: string[]): boolean {
		...
		if (info === null) this.error("Invalid access token provided")
			...
			this.error(
				`Invalid application kind: ${clColor.msg.error(info.application.kind)}. Application kind must be one of the following: ${clColor.cyanBright(kinds.join(", "))}`,
			)
	...
	checkResourceId(
		...
		if (si >= 0) {
			...
			if (id && rt[1])
				this.error(`Double definition of resource id: [${res}, ${id}]`, {
					suggestions: [
						`Define resource id as command argument (${clColor.italic(id)}) or as part of the resource itself (${clColor.italic(res)}) but not both`,
					],
				})
			else id = rt[1]
			res = rt[0]
		...
		if (id) {
			if (singleton)
				this.error(
					`Singleton resource ${clColor.api.resource(res)} does not require id`,
				)
			if (id.includes("/"))
				this.error(`Invalid resource id: ${clColor.msg.error(id)}`)
	...
	checkResource(
		res: string,
		{ required = true, singular = false } = {},
	): Resource {
		if (!res && required) this.error("Resource type not defined")
		const resource = findResource(res, { singular })
		if (resource === undefined)
			this.error(`Invalid resource ${clColor.style.error(res)}`, {
				suggestions: [
					`Execute command ${clColor.style.command("resources")} to get a list of all available CLI resources`,
				],
			})
		return resource
	}

	protected initCommerceLayer(
		flags: any,
		...options: any[]
			...
			if (name === "update" && attributes) {
				if (
					!Object.keys(attributes).some(
						(attr) =>
							!["reference", "reference_origin", "metadata"].includes(attr),
					)
				)
					return true
			}
			this.error(
				`Operation not supported for resource ${clColor.api.resource(sdk.type())}: ${clColor.msg.error(name)}`,
			)
		}
		return true
	}
	...
	printHeaders(headers: any, flags: any): void {
		if (headers) {
			if (flags.headers || flags["headers-only"]) {
				...
				if (Object.keys(headers as object).length === 0)
					this.log(clColor.italic("No headers"))
	...
	printError(error: any, flags?: any, args?: any): void {
		...
		if (CommerceLayerProvisioningStatic.isApiError(err)) {
			err =
				err.errors ||
				`Unable to find resource of type ${clColor.msg.error(args.resource)} and id ${clColor.msg.error(args.id)}`
			...
			if (error.response.status === 401)
				...
				this.error(
					`We're sorry, but something went wrong (${error.response.status})`,
				)
			else if (error.response.status === 429)
				this.error(
					`You have done too many requests in the last 5 minutes (${error.response.status})`,
				)
			else err = error.response.data.errors
		} else if (error.message) err = error.message
	...
	_keyvalFlag(flag: string[] | undefined, type = "attribute"): KeyValString {
		...
		if (flag && flag.length > 0) {
			flag.forEach((f) => {
				const eqi = f.indexOf("=")
				if (eqi < 1)
					this.error(`Invalid ${type.toLowerCase()} ${clColor.msg.error(f)}`, {
						suggestions: [
							`${clText.capitalize(type)} flags must be defined using the format ${clColor.cli.value("name=value")}`,
						],
					})
				...
				if (param[name])
					this.warn(
						`${clText.capitalize(type)} ${clColor.msg.warning(name)} has already been defined`,
					)
...
export abstract class BaseFilterCommand extends BaseCommand {
    ...
    */
	...
	async catch(error: CommandError): Promise<any> {
		if (error.message?.match(/Missing \d required args?:\nresource/))
			this.error(`Missing argument ${clColor.style.error("resource")}`, {
				suggestions: [
					`Execute command ${clColor.style.command("resources")} to get a list of all available Provisioning resources`,
				],
			})
	...
	includeFlag(
		...
		if (flag) {
			...
			if (values.some((f) => f.split(".").length > 3) && !force)
				this.error(
					"Can be only included resources within the 3rd level of depth",
				)
		}
	...
	objectFlag(flag: string[] | undefined): KeyValObj {
		...
		if (flag && flag.length > 0) {
			flag.forEach((f) => {
				...
				if (slashSep < 0)
					this.error(
						`No name or fields defined in flag object${clColor.style.flag(f)}`,
					)
				...
				if (fields[0].trim() === "")
					this.error(
						`No fields defined for object field ${clColor.style.attribute(name)}`,
					)
				...
				fields.forEach((f) => {
					...
					if (eqi < 0)
						this.error(
							`No value defined for object field ${clColor.style.attribute(f)} of object ${clColor.style.attribute(name)}`,
						)
		...
		if (flag && flag.length > 0) {
			flag.forEach((f) => {
				...
				if (f.includes("/")) {
					...
					if (kv.length > 2)
						this.error(
							"Can be defined only one resource for each fields flag",
							{
								suggestions: [
									`Split the value ${clColor.style.attribute(f)} into two fields flags`,
								],
							},
						)
				...
				if (values[0].trim() === "")
					this.error(
						`No fields defined for resource ${clColor.api.resource(res)}`,
					)
	...
	whereFlag(flag: string[] | undefined): KeyValString {
		...
		if (flag && flag.length > 0) {
			flag.forEach((f) => {
				...
				if (wt.length < 2)
					this.error(
						`Filter flag must be in the form ${clColor.style.attribute("predicate=value")}`,
					)
				const w = wt[0]
				if (!clFilter.available(w))
					this.error(`Invalid query filter: ${clColor.style.error(w)}`, {
						suggestions: [
							`Execute command ${clColor.style.command("resources:filters")} to get a full list of all available filter predicates`,
						],
				...
				flag.forEach((f) => {
					...
					if (ot.length > 2)
						this.error("Can be defined only one field for each sort flag", {
							suggestions: [
								`Split the value ${clColor.style.attribute(f)} into two or more sort flags`,
							],
						})
					...
					if (of.startsWith("-"))
						this.error("You cannot mix two ordering syntaxes", {
							suggestions: [
								`Choose between the style ${clColor.cli.value("<field>,<order>")} and the style ${clColor.cli.value("[-]<field>")}`,
							],
						})
					const sd = ot[1] || "asc"
					if (!["asc", "desc"].includes(sd))
						this.error(`Invalid sort flag: ${clColor.msg.error(f)}`, {
							suggestions: [
								`Sort direction can assume only the values ${clColor.cli.value("asc")} or ${clColor.cli.value("desc")}`,
							],
						})
		...
		if (flag && flag.length > 0) {
			flag.forEach((f) => {
				...
				if (rt.length === 2) {
					...
					this.error(`Invalid relationship flag: ${clColor.msg.error(f)}`, {
						suggestions: [
							`Define the relationship using the format ${clColor.cli.value("attribute_name=resource_type/resource_id")}`,
						],
					})
				...
				if (relationships[name])
					this.warn(
						`Relationship ${clColor.yellow(name)} has already been defined`,
					)
			...
			flag.forEach((f) => {
				...
				if (kv.length > 2)
					this.error("Can be defined only one field for each extract flag", {
						suggestions: [
							`Split the value ${clColor.cli.value(f)} into two extract flags`,
						],
					})
				else if (kv.length === 1)
					this.error(
						`No fields defined for object ${clColor.cli.value(kv[0])}`,
					)
				...
				if (fields[0].trim() === "")
					this.error(
						`No fields defined for object field ${clColor.cli.value(name)}`,
					)
	...
	extractObjectFields(fields: KeyValArray, obj: any): void {
		Object.entries(fields).forEach(([extObj, extFields]) => {
			...
			if (curObj && typeof curObj === "object" && !curObj.id && !curObj.type)
				...
				for (const k of Object.keys(curObj as object))
					if (!extFields.includes(k)) delete curObj[k]
		})
	}
	...
	saveOutput(output: any, flags: any): void {
		try {
			...
			fileExport(output, flags, filePath)
				...
				.catch(() =>
					this.error(
						`Unable to save command output to file ${clColor.style.path(filePath)}`,
						{
							suggestions: [
			...
			if (error.code === "ENOENT")
				this.warn(
					`Path not found ${clColor.msg.error(error.path)}: execute command with flag ${clColor.cli.flag("-X")} to force path creation`,
				)
			else throw error
		} finally {
			this.log()
	...
	protected async showLiveDocumentation(
		request: RequestData,
		params?: QueryParams,
		flags?: any,
	): Promise<string> {
